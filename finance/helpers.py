...
def update_portfolio(buycode, id, symbol, shares, db): # buycode 1 for buy, 0 for sell. RETURN 1 on error, insufficient shares to sell
    ...
    if len(row) == 0:
        ...
        if currentshares == 0:
            db.execute("DELETE FROM portfolio WHERE user_id = ? AND sym_id = ?;", id, sym_id)
        else:
...
def return_transactions(arguserid, argdb):
    rows = argdb.execute("SELECT * FROM symbol, transactions WHERE symbol.id = transactions.sym_id AND user_id = ? ORDER BY date ASC, time ASC;", arguserid)
    return rows
